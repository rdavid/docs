...
{url-operators}[Red Hat's operators] on OCP {ocp-version}. As a result, we
identified a total of over seven thousand failures. You can find more
information in {url-reports}[the report].
...
- {url-tainted}[`platform-alteration-tainted-node-kernel`]
